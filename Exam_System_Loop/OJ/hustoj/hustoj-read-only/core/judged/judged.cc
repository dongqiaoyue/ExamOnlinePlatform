...
//Redis
static redisContext *oj_rediscli = NULL;
static redisReply *redisres;
...
int init_redis() {
	printf("get init redis");
	if (oj_rediscli == NULL) {
		oj_rediscli = redisConnect(oj_redisserver, oj_redisport);

		if (oj_rediscli == NULL || oj_rediscli->err) {
			if (DEBUG)
				write_log("%s", mysql_error(conn));
			sleep(2);
			return 1;
...
int _get_jobs_redis(int * jobs){
	...
	while (i < max_running * 2) {
		redisres = (redisReply *)redisCommand(oj_rediscli,
			"lpop solutionList");

		if (redisres->type == 4) {
			break;
		} else {
			jobs[i++] = atoi(redisres->str);
		}
	}
	ret = i;
	while (i <= max_running * 2)
...
bool _check_out_redis(int solution_id, int result) {
	redisres = (redisReply *)redisCommand(oj_rediscli,
		"hmset solution_redis:a:%d result %d", solution_id, result);
...
bool check_out(int solution_id, int result) {
	if (http_judge) {
		...
		return _check_out_redis(solution_id, result);
...
//	final_sleep.tv_nsec=500000000;
	...
	while (1) {			// start to run
		if (oj_redis) {
			init_status = init_redis();
		} else {
			init_status = init_mysql();
		}
		while (j && (http_judge || !init_status)) {
