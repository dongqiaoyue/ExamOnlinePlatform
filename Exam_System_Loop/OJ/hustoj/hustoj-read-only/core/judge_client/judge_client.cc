...
void init_mysql_conf() {
	...
	if (fp != NULL) {
		while (fgets(buf, BUFFER_SIZE - 1, fp)) {
            ...
            read_int(buf, "OJ_KEEPLIVE", &oj_keeplive);
...
void _update_solution_redis(int solution_id, int result, int time, int memory,
	...
	// oj_rediscli = redisConnect("127.0.0.1", 6379);

	redisReply *res = (redisReply *)redisCommand(oj_rediscli,
		"hmset solution_redis:a:%d time %d memory %d result %d",
		solution_id, time, memory, result);

	printf("\n%s", res->str);

	// TO DO EXPIRE
	// res = (redisReply *)redisCommand(oj_rediscli, "EXPIRE solution_redis:a:%d %d",
	// 	solution_id, oj_keeplive);
...
void _addceinfo_redis(int solution_id) {
	...
	redisReply *res = (redisReply *)redisCommand(oj_rediscli,
		"hmset solution_redis:a:%d compile_info %s", solution_id, ceinfo);
...
void _addreinfo_redis(int solution_id, const char * filename) {
	...
	redisReply *res = (redisReply *)redisCommand(oj_rediscli,
		"hmset solution_redis:a:%d runtime_info %s", solution_id, reinfo);
...
void _get_solution_redis(int solution_id, char * work_dir, int lang) {
	...
	redisReply *res = (redisReply *)redisCommand(oj_rediscli,
		"hmget solution_redis:a:%d source_code", solution_id);
	redisReply **elements = res->element;

	printf("get solution code");
	char *source_code = elements[0]->str;
...
void _get_solution_info_redis(int solution_id, int & p_id, char * user_id,
	...
	// Get solution info
	redisReply *res = (redisReply *)redisCommand(oj_rediscli,
		"hgetall solution_redis:a:%d", solution_id);


	redisReply **elements = res->element;

	for (int i = 0; i < res->elements ; i++) {
		if (!strcmp(elements[i]->str, "problem_id")) {
			p_id = atoi(elements[++i]->str);
		} else if (!strcmp(elements[i]->str, "user_id")) {
			user_id = elements[++i]->str;
		} else if (!strcmp(elements[i]->str, "language")) {
			lang = atoi(elements[++i]->str);
		} else {
			i++;
		}
	}

	// p_id = atoi(elements[19]->str);
	// user_id = elements[1]->str;
	// lang = atoi(elements[13]->str);

	printf("language: %d", lang);
	printf("problem_id: %d", p_id);
...
void _get_problem_info_redis(int p_id, int & time_lmt, int & mem_lmt,
	...
	// Get problem_id
    redisReply *res = (redisReply *)redisCommand(oj_rediscli,
		"hmget solution_redis:a:%d problem_id", solution_id);
	int problem_id = atoi(res->element[0]->str);

	// Get problem info
	res = (redisReply *)redisCommand(oj_rediscli,
		"hgetall problem_redis:a:%d", problem_id);

	redisReply **elements = res->element;

	for (int i = 0; i < res->elements ; i++) {
		if (!strcmp(elements[i]->str, "spj")) {
			isspj = atoi(elements[++i]->str);
		} else if (!strcmp(elements[i]->str, "time_limit")) {
			time_lmt = atoi(elements[++i]->str);
		} else if (!strcmp(elements[i]->str, "memory_limit")) {
			mem_lmt = atoi(elements[++i]->str);
		} else {
			i++;
		}
	}
	...
	// isspj = atoi(elements[5]->str);
	// time_lmt = atoi(elements[7]->str);
	// mem_lmt = atoi(elements[9]->str);
