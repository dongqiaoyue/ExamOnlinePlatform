                            ...
                            <div class="col-xs-6 col-xs-offset-3">
                                <table class="table table-bordered">
                                    ...
                                    <tr>
                                        <td>题目数量</td>
                                        <td>
                                            <?php foreach ($stage as $k=>$value){?>
                                                <label class="checkbox-inline">
                                                    <input type="checkbox" id="" value=""><?=$value["KnowledgeName"]?>
                                                </label>
                                            <?php }?>
                                        </td>
                                    </tr>
...
<script>
    ...
    $('#question-type-add').click(function (e) {
        ...
        if(Tmp_One != true && Tmp_Code!='0') {
            ...
            $('#question-type-text').append('<div class="tab-pane fade  row" id="'+ Tmp_Code +'">' +
                ...
                '<tr><td>题目难度</td>' +
                '<td><?php foreach ($diff as $value){?><label class="checkbox-inline" id="'+ Tmp_Code +'-diff"><input type="checkbox" id="'+ Tmp_Code +'-'+ <?=$value["CuitMoon_DictionaryCode"]?> +'-diff" value="<?=$value["CuitMoon_DictionaryCode"]?>" onclick="Diff('+ Tmp_Code +',<?=$value["CuitMoon_DictionaryCode"]?>)"><?=$value["CuitMoon_DictionaryName"]?></label><?php }?></td></tr><tr><td>题目阶段</td><td><?php foreach ($stage as $k=>$value){?><label class="checkbox-inline" id="'+ Tmp_Code +'-stage"><input type="checkbox" name="Stages['+ Tmp_Code +'][]" id="'+ Tmp_Code +'-stage" value="<?=$value["KnowledgeBh"]?>" onclick="Stage('+ Tmp_Code +')"><?=$value["KnowledgeName"]?></label><?php }?></td></tr><tr><td>每种难度题目数量</td><td><table class="table" id="'+ Tmp_Code +'-new-table">' +
    ...
    $('#examModuleAdd').bind('submit',function (e) {
        ...
        $(this).ajaxSubmit({
            type:'POST',
            url:'<?=Url::toRoute("exam-module/create")?>&id='+'<?= $id?>',
            dataType:'JSON',
            traditional: true,
            success:function (value) {
                //alert(value);
